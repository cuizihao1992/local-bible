...
assert(appJs.includes("let currentAiResultMeta = null;"), "AI result metadata state missing");
...
assert(appJs.includes("已复制笔记") && appJs.includes("标签：${tags}"), "Inline note copy feedback missing");
...
assert(appJs.includes("copyVerseNote(verseNo, { note, tags })") && appJs.includes("const note = draft.note ?? mark.note"), "Inline note copy should prefer editor draft");
