...
public class OfflineApi {
    ...
    private static final String RELEASE_BASE = "https://github.com/cuizihao1992/local-bible/releases/download/v1.9.29/";
    ...
    private JSONArray packages() throws Exception {
        ...
        array.put(packageInfo("extra-bibles", "更多译本", "下载 22 个补充经文译本", "bibles-extra-v1.9.29.zip", bibleDir, 26));
        array.put(packageInfo("commentaries", "注释库", "下载 18 个注释数据库", "commentaries-v1.9.29.zip", commentaryDir, 18));
        return array;
    }
    ...
    private JSONObject installDataPackage(String packageId) throws Exception {
        ...
        if ("extra-bibles".equals(packageId)) {
            fileName = "bibles-extra-v1.9.29.zip";
            targetDir = bibleDir;
        } else if ("commentaries".equals(packageId)) {
            fileName = "commentaries-v1.9.29.zip";
        ...
        connection.setRequestProperty("User-Agent", "LocalBibleReader/1.9.29");
    ...
    private JSONObject progress(String version) throws Exception {
        ...
        return new JSONObject().put("version", version).put("total", total).put("read", read.length()).put("percent", total == 0 ? 0 : Math.round(read.length() * 1000.0 / total) / 10.0).put("readChapters", read).put("books", books());
    }
    ...
    private JSONObject importUserData(JSONObject body) throws Exception {
        ...
        return new JSONObject().put("imported", imported).put("progressImported", 0);
    }

    private JSONObject health() throws Exception {
        return new JSONObject().put("ok", true).put("platform", "android-offline").put("versionCount", versions().length());
    }
    ...
    private String[] append(List<String> list, String value) {
